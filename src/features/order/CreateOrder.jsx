...
function CreateOrder() {
  ...
  const {
    ...
    address,
    error: errorAddress,
  } = useSelector((state) => state.user); // how we read value from redux store
      ...
      <Form method="POST">
        ...
        <div className="relative mb-5 flex flex-col gap-2 sm:flex-row sm:items-center">
          ...
          {!position.latitude && !position.longtitude && (
            <span className="absolute right-[3px] top-0  z-50 sm:right-[3px] sm:top-[3px] md:right-[5px] md:top-[5px]">
              <Button
                disabled={isLoadingAdress}
                type="small"
                onClick={(e) => {
...
export async function action({ request }) {
  ...
  // Do NOT overuse, deactivate some performance optimization of redux on this page
  store.dispatch(clearCart());

  // hook can only be called inside component
